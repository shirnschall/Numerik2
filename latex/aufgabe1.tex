...
\lstset{ %
	...
	otherkeywords={int,unsigned,long,char,template,typename,operator,nullptr,new,this,*,...},           % if you want to add more keywords to the set
	...
	emph={vector,string,linag,Vector,DenseMatrix,SparseMatrix,std,Eigen},
	emphstyle={\color{ballblue}}
}
...
\begin{document}
	...
	\lstinputlisting[language=C++, firstline = 134, lastline= 153, caption = Erstellen einer symmertrisch postiv definiten Zufallsmatrix mit einer fixen Anzahl an Einträgen ungleich $0$ pro Zeile in C++]{../code/Linag/densematrix.h} 
	...
	\lstinputlisting[language=C++, firstline = 499, lastline= 548, caption = Implementierung des CG-Verfahrens in C++]{../code/Linag/densematrix.h} \label{lst:cg}
	...
	\lstinputlisting[language=C++, firstline = 254, lastline= 288, caption = Speicherung einer Matrix im compressed spare row Format]{../code/Linag/sparsematrix.h} 
	
	\lstinputlisting[language=C++, firstline = 576, lastline= 592, caption = compressed spare row Format zu vollbesetzter Matrix]{../code/Linag/densematrix.h} 
	
	
	Die Implementierung des CG-Verfahrens unterscheidet sich dabei nicht von \cref{lst:cg}. Die Matrix-Vektor Multiplikation kann f\"ur d\"unnbesetzte Matrizen jedoch effizienter implementiert werden.  (\cref{code:sparse-vektor-mult})
	\lstinputlisting[language=C++, firstline = 84, lastline= 95, caption = \"Uberladen der Matrix-Vektor Multiplikation f\"ur d\"unnbesetzte Matrizen]{../code/Linag/sparsematrix.h} \label{code:sparse-vektor-mult}
	...
	\end{subfigure}
		\begin{subfigure}[t]{0.5\textwidth}
			...
			\subcaption{Koeffizientenmatrix der Form $A = B+B^T+c\cdot I$}
			\label{fig:precond-vs-cg-ndiag}	
		\end{subfigure}
	
	\caption{Residuum nach Iterationsschritt $t$ f\"ur vorkonditioniertes CG- und CG-Verfahren}\label{fig:vcg-vergleich}
	\end{figure}


	F\"ur Matrizen bei denen das Vorkonditionieren gut funktioniert, bleibt die Iterationszahl f\"ur verschieden gro\ss e Probleme nahezu konstant und die Streuung wird geringer, wie an den Trendlinien und den Konfidenzintervallen in \cref{fig:precond-vs-cg-it} zu sehen ist.
