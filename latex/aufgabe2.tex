...
\begin{document}
	...
	\begin{algorithm}[H]
		...
		\begin{algorithmic}[1]
			...
			\State $x_{t+1} = x_{t+1} + \alpha_t d_t$
			\State $r_{t+1} = r_t - \alpha_t Ad_t$
			\State $\beta_k = \frac{r_{t+1}^{T}r_{t+1}}{r_t^{T}r_t}$
	...
	Nun stellt sich die Frage, ob \cref{alg:cg} äquivalent zum Algorithmus \autocite[vgl.][101]{skript} und welches bevorzugt wird.
	
	\newpage
	Die spektrale Konditionszahl $cond(A) = definition$ definiert die Konvergenzgeschwindigkeit des CG-Verfahrens. Durch lösen des vorkonditionierten Systems
	\begin{align*}
		D^{-1}AD^{-T}y = D^{-1}b
	\end{align*}
	...
	\begin{algorithm}[H]
		...
		\begin{algorithmic}[1]
			...
			\State $\beta_k = \frac{z_{t+1}^{T}r_{t+1}}{z_tr_t}$
			\State $d_{t+1} = r_{t+1} + \beta_td_t$
			\State $t = t + 1$
			\EndWhile
		\end{algorithmic}
